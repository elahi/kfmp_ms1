################################################################################
##' @title Process raw data, lump2 categories
##' @author Robin Elahi
##' @date 2026-03-31
##' @log 
################################################################################
...
#### File paths ####
here::i_am("kfmp_transect_video_species/_process_raw_data_lump2.R")
library(here)
folder <- "kfmp_transect_video_species"
file_name <- "_process_raw_data_lump2.R"
...
# lump2
dat <- dat |> 
  mutate(taxon = ifelse(is.na(lump2), taxon, lump2))
glimpse(dat)
...
table1 <- dat |> select(taxon, phylum, trophic_level) |> 
  mutate(name = gsub("_", " ", taxon), 
         view = ifelse(phylum == "Vertebrate", "Kelp", "Benthic")) 
